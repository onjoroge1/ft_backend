...
app.post('/convertFromPDF', pdfToDocx.single("file"), (req, res) => {
    ...
    PDFNet.runWithCleanup(convert, "demo:1661285733817:7a0b0ed80300000000ac11f86f4ce6d72581ce80dcdfd70123c396b191")
        .then(() => {
...
app.post('/convertFromOFFICE', officeToPDF.single("file"), (req, res) => {
    ...
    PDFNet.runWithCleanup(convertToPDF, "demo:1661285733817:7a0b0ed80300000000ac11f86f4ce6d72581ce80dcdfd70123c396b191")
        .then(() => {
            fs.readFile(outputPath, (err, data) => {
                if (err) {
                    res.statusCode = 500;
...
// --- PDF TO EXCEL START --- //
app.post('/convertFromPDFtoEXCEL', pdfToExcel.single("file"), async (req, res) => {
    let filePath = `./${req.file.path}`;
...
// --- PDF TO HTML START --- //
app.post('/convertFromPDFtoHTML', pdfToHtml.single("file"), async (req, res) => {
    let filePath = `./${req.file.path}`;
